...
export default function TxPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        const response = await fetch(`/api/transaction/${hash}`, { cache: "no-store" })
        const text = await response.text()
        const data = text.trim() ? JSON.parse(text) : null
        if (!response.ok || !data?.transaction) {
          setError(data?.error || `Transaction not found on ${ARC_CHAIN_NAME}`)
          return
        }
        setTx(data.transaction as TxDetail)
      } catch (e) {
  ...
  const statusTone = tx?.status === "success"
    ? { background:"rgba(0,184,122,0.08)", color:"#00d990", border:"rgba(0,184,122,0.2)" }
    : tx?.status === "pending"
      ? { background:"rgba(224,160,32,0.08)", color:"#e0a020", border:"rgba(224,160,32,0.2)" }
      : { background:"rgba(224,51,72,0.08)", color:"#e03348", border:"rgba(224,51,72,0.2)" }

  function Row({ label, value, link, color }: { label:string; value:string; link?:string|null; color?:string }) {
    return (
      <div className="tx-detail-row" style={{ display:"flex", alignItems:"flex-start", gap:"20px", padding:"11px 20px", borderBottom:"1px solid rgba(128,128,128,0.04)" }}>
        <div className="tx-detail-label" style={{ fontSize:"10px", fontFamily:mono, color:t3, textTransform:"uppercase", letterSpacing:"0.08em", minWidth:"130px", flexShrink:0, paddingTop:"2px" }}>{label}</div>
        {link ? (
  ...
  return (
    <ArcLayout active="">
      <div className="tx-page" style={{ padding:"28px 28px 48px" }}>
          ...
          <>
            ...
            <div style={{ marginBottom:"24px" }}>
              ...
              <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
                <span style={{ fontSize:"10px", fontFamily:mono, padding:"3px 10px", borderRadius:"5px", background:statusTone.background, color:statusTone.color, border:"1px solid "+statusTone.border }}>
            ...
            {/* SUMMARY BAND */}
            <div className="tx-summary" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"1px", background:bdr, borderRadius:"14px", overflow:"hidden", border:"1px solid "+bdr, marginBottom:"20px" }}>
              {[
            ...
            {/* VALUE FLOW */}
            {(tx.from || tx.to) && (
              <div className="tx-section tx-flow-section" style={{ background:surf, border:"1px solid "+bdr, borderRadius:"14px", padding:"20px", marginBottom:"16px" }}>
                ...
                <div className="tx-value-flow" style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                  {/* FROM */}
                  <div className="tx-party" onClick={() => window.location.href="/address/"+tx.from} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"6px", cursor:"pointer" }}>
                  ...
                  {/* Arrow + amount */}
                  <div className="tx-flow-leg" style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"4px" }}>
                  ...
                  {tx.isUSDCTransfer && (
                    <>
                      <div className="tx-party" onClick={() => window.location.href="/address/0x3600000000000000000000000000000000000000"} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"6px", cursor:"pointer" }}>
                        <div style={{ width:"44px", height:"44px", borderRadius:"50%", overflow:"hidden", border:"2px solid rgba(0,184,122,0.3)", flexShrink:0 }}>
                          <img src="/tokens/usdc.svg" alt="USDC" style={{ width:"100%", height:"100%" }} />
                        </div>
                  ...
                  {/* TO */}
                  {tx.to && (
                    <div className="tx-party" onClick={() => window.location.href="/address/"+tx.to!} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"6px", cursor:"pointer" }}>
            ...
            {/* DETAILS */}
            <div className="tx-section" style={{ background:surf, border:"1px solid "+bdr, borderRadius:"14px", overflow:"hidden", marginBottom:"16px" }}>
              <div className="tx-section-heading" style={{ padding:"13px 20px", borderBottom:"1px solid "+bdr, display:"flex", alignItems:"center", gap:"8px" }}>
              ...
              <Row label="Status"     value={tx.status === "success" ? "Success" : tx.status === "pending" ? "Pending" : "Failed"} color={tx.status==="success"?"#00d990":tx.status==="pending"?"#e0a020":"#e03348"} />
            </div>
              ...
              <div className="tx-section" style={{ background:surf, border:"1px solid "+bdr, borderRadius:"14px", overflow:"hidden", marginBottom:"16px" }}>
                ...
                <div style={{ padding:"16px 20px" }}>
                  ...
                  {tx.decodedInput.parameters.map((p, i) => (
                    <div className="tx-decoded-row" key={i} style={{ display:"flex", gap:"16px", padding:"8px 0", borderBottom:"1px solid rgba(128,128,128,0.04)" }}>
            ...
            {tx.inputData && tx.inputData !== "0x" && !tx.decodedInput && (
              <div className="tx-section" style={{ background:surf, border:"1px solid "+bdr, borderRadius:"14px", overflow:"hidden" }}>
                <div className="tx-section-heading" style={{ padding:"13px 20px", borderBottom:"1px solid "+bdr, display:"flex", alignItems:"center", gap:"8px" }}>
        ...
        <style>{`
          @media (max-width: 640px) {
            .tx-page { padding: 20px 18px 36px !important; }
            .tx-summary {
              grid-template-columns: repeat(2, minmax(0, 1fr)) !important;
              background: transparent !important;
              border: 0 !important;
              border-radius: 0 !important;
              border-top: 1px solid var(--bdr, rgba(255,255,255,0.06)) !important;
              border-bottom: 1px solid var(--bdr, rgba(255,255,255,0.06)) !important;
            }
            .tx-summary > div { padding: 15px 10px !important; min-width: 0; background: transparent !important; }
            .tx-summary > div:nth-child(odd) { border-right: 1px solid var(--bdr, rgba(255,255,255,0.06)); }
            .tx-summary > div:nth-child(-n+2) { border-bottom: 1px solid var(--bdr, rgba(255,255,255,0.06)); }
            .tx-summary > div > div:last-child { font-size: 15px !important; overflow-wrap: anywhere; }
            .tx-section {
              background: transparent !important;
              border: 0 !important;
              border-radius: 0 !important;
            ...
            .tx-flow-section { padding: 18px 0 !important; }
            .tx-section-heading { padding: 14px 0 !important; }
            .tx-value-flow { flex-direction: column !important; align-items: stretch !important; gap: 10px !important; }
            .tx-party { flex-direction: row !important; justify-content: flex-start; min-width: 0; }
            .tx-party > div:last-child { overflow-wrap: anywhere; }
            .tx-flow-leg { width: 100%; flex: none !important; padding: 2px 0; }
            .tx-detail-row { flex-direction: column !important; gap: 5px !important; padding: 12px 0 !important; }
            .tx-detail-label { min-width: 0 !important; }
            .tx-detail-row > a, .tx-detail-row > div:last-child { width: 100%; font-size: 11.5px !important; }
            .tx-decoded-row { flex-direction: column !important; gap: 5px !important; }
          }
        `}</style>
      </div>
    </ArcLayout>
