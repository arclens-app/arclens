...
const CHAINS: Record<number, { name: string; logo: string; color: string }> = {
...
export default function BridgePage() {
  ...
  const [cumVolume, setCumVolume]     = useState(0)

  // ref to track last refresh time for the live counter
  const lastRefreshRef = useRef<number>(Date.now())

  useEffect(() => { setMounted(true) }, [])

  const computeStats = useCallback((items: Transfer[], totalIn = 0, totalOut = 0, resetVol = true) => {
    ...
    setStats({
      totalIn:     inItems.reduce((s,t) => s + t.amountNum, 0),
      totalOut:    outItems.reduce((s,t) => s + t.amountNum, 0),
  ...
  const loadBridgeData = useCallback(async () => {
    try {
      ...
      for (const t of inItems) {
        ...
        seen.add(hash)
        if (t.method !== "receiveMessage") continue
        ...
        try {
          ...
          const msgParam = params.find((p: {name:string;value:string}) => p.name === "message")
          if (msgParam?.value) {
        ...
        parsed.push({ hash, from: fromAddr, to: fromAddr, amount: amountStr, amountNum, srcDomain, dstDomain: ARC_DOMAIN, timestamp: ts, direction: "in", status: t.result === "success" ? "complete" : "pending" })
      }
      ...
      for (const t of outItems) {
        ...
        const decoded   = t.decoded_input as Record<string,unknown>
        const params    = decoded?.parameters as {name:string;value:string}[] || []
        const amountParam = params.find((p: {name:string;value:string}) => p.name === "amount")
        ...
        const destDomainParam = params.find((p: {name:string;value:string}) => p.name === "destinationDomain")
        const dstDomain = destDomainParam ? Number(destDomainParam.value) : 0
        parsed.push({ hash, from: fromAddr, to: fromAddr, amount: amountStr, amountNum, srcDomain: ARC_DOMAIN, dstDomain, timestamp: ts, direction: "out", status: t.result === "success" ? "complete" : "pending" })
      }
      ...
      setAllTimeIn(Number(inCount?.transactions_count  || 0))
      setAllTimeOut(Number(outCount?.transactions_count || 0))

      if (inData.next_page_params)  setHasMoreIn(true)
      if (outData.next_page_params) setHasMoreOut(true)

      parsed.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
      setTransfers(parsed)
      setPage(1)
      computeStats(parsed, Number(inCount?.transactions_count || 0), Number(outCount?.transactions_count || 0), true)
      setHasLoaded(true)
      setLastUpdate(new Date())
      lastRefreshRef.current = Date.now()
      setTimeSince(0)
  ...
  function ChainBadge({ domain, size = 20 }: { domain: number; size?: number }) {
    ...
    return (
      <div style={{ display:"flex", alignItems:"center", gap:"5px" }}>
        <img src={chain.logo} alt={chain.name} style={{ width:size+"px", height:size+"px", borderRadius:"50%", objectFit:"cover", flexShrink:0 }} onError={e => (e.currentTarget.style.display="none")} />
      ...
      <div style={{ padding:"28px 28px 48px" }}>
        ...
        <div style={{ marginBottom:"24px", display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:"12px" }}>
          <div>
            ...
            <div style={{ fontSize:"13px", color:t2, fontWeight:300 }}>Live USDC flows in and out of Arc via Circle CCTP V2.</div>
          </div>
          <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:"6px" }}>
            <button onClick={() => { setRefreshing(true); loadBridgeData() }} disabled={refreshing}
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:"10px", alignItems:"center" }}>
            {[0,1,2,3,6,8,10,11,14].map(domain => (
              <div key={domain} style={{ display:"flex", alignItems:"center", gap:"6px", padding:"6px 12px", background:surf2, borderRadius:"8px", border:"1px solid "+bdr }}>
                <img src={CHAINS[domain]?.logo} alt={CHAINS[domain]?.name} style={{ width:"16px", height:"16px", borderRadius:"50%", objectFit:"cover" }} onError={e => (e.currentTarget.style.display="none")} />
        ...
        <div style={{ display:"flex", gap:"8px", marginBottom:"16px" }}>
          {[
            ...
            <button key={f.id} onClick={() => setFilter(f.id)}
              ...
              {f.label}
              {f.id !== "all" && <span style={{ marginLeft:"5px", fontSize:"10px", opacity:.7 }}>({f.id === "in" ? transfers.filter(t=>t.direction==="in").length : transfers.filter(t=>t.direction==="out").length})</span>}
            </button>
          ))}
        </div>
        ...
        <div style={{ background:surf, border:"1px solid "+bdr, borderRadius:"14px", overflow:"hidden" }}>
          ...
          {loading ? (
            <div style={{ padding:"60px", textAlign:"center", fontFamily:mono, fontSize:"11px", color:t3 }}>Loading bridge transfers...</div>
          ) : filtered.length === 0 ? (
          ...
          {(page > 1 || hasMoreIn || hasMoreOut) && (
            <div style={{ padding:"14px 18px", borderTop:"1px solid "+bdr, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div style={{ fontSize:"10px", fontFamily:mono, color:t3 }}>Showing page {page} · {filtered.length} transfers</div>
              <div style={{ display:"flex", gap:"8px" }}>
                ...
                <div style={{ height:"32px", padding:"0 14px", background:"rgba(26,86,255,0.08)", color:"#8aaeff", fontSize:"12px", fontFamily:mono, border:"1px solid rgba(26,86,255,0.2)", borderRadius:"7px", display:"flex", alignItems:"center" }}>{page}</div>
                <button disabled={(!hasMoreIn && !hasMoreOut)||loadingPage}
