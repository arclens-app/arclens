...
export async function POST(req: NextRequest) {
  ...
  try {
    // Check if this is an update — same contract + same email
    if (contract?.trim()) {
      ...
      if (existing.rows.length > 0) {
        ...
        if (existingEmail === submittedEmail) {
          // Same email + same contract = update request
          await pool.query(
            `UPDATE projects SET
    ...
    // New submission
    await pool.query(
      `INSERT INTO projects (name, tagline, description, category, logo_url, website, twitter, github, discord, contract, email, approved, live)
